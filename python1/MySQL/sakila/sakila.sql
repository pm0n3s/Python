...
-- 2. What query would you run to get all comedy films? Your query should return film 
...
SELECT actor.actor_id, CONCAT(actor.first_name, ' ', actor.last_name) AS name, film.title, 
...
-- feature = behind the scenes", joined by actor_id = 15? Your query should return the film 
-- title, description, release year, rating, and special feature. Hint: You may use LIKE 
-- function in getting the 'behind the scenes' part.
...
-- query should return film title, description, release year, rating, special features, and 
-- genre (category).
SELECT film.title, film.description, film.release_year, film.rating, film.special_features,
category.name
FROM film
...
-- 8. What query would you run to get all the action films which are joined by SANDRA 
-- KILMER? Your query should return film title, description, release year, rating, special 
-- features, genre (category), and actor's first name and last name.
SELECT film.title, film.description, film.release_year, film.rating, film.special_features,
category.name, CONCAT(actor.first_name, ' ', actor.last_name)
FROM film
	JOIN film_category ON film.film_id = film_category.film_id
    JOIN category ON film_category.category_id = category.category_id
    JOIN film_actor ON film.film_id = film_actor.film_id
...
AND CONCAT(actor.first_name, actor.last_name) = 'SANDRAKILMER';
